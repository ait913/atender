...
|---|---|---|---|---|---|
| GET | `/api/me` | @auth | (none) | `{ user: UserDto, setupStatus: { hasSchool, hasDepartment, hasSemester, isComplete } }` | 401 |
...
z.object({
  ...
  schoolId: z.string().nullable(),       // 直近 UserTimetable から導出
...
| DELETE | `/api/semesters/:id` | @auth | (none) | `{ ok: true }` (204) | 401 / 404 / 403 / 409 (UserTimetable に紐付いている場合) |
...
| POST | `/api/user-timetables/:id/publish-as-template` | @auth | body: `{ title: string, description?: string, year?: number, term?: string }` | `{ template: TemplateDto }` | 401 / 403 / 404 (school/dept 未設定) |

`POST /api/user-timetables/:id/publish-as-template`: ユーザーの時間割を Template として公開する F6 機能。`school_id` / `department_id` は user の直近セットアップから引く。
...
3 step とも左に「戻る」、右に「次へ」。完了時に POST /api/semesters → User.defaultSemesterId に紐付け → `/timetable` へ。
...
- AttendanceRule
  ...
  - 自分の上書き
- 学校・学科変更 (UserTimetable 紐付けは消えないが Department 変更時は警告)
- サインアウト
...
sequenceDiagram
  ...
  BA->>BA: token 発行、Verification 行作成 (expiresAt = now + 15 min)
  BA->>R: resend.emails.send({from, to: email, html: "<a href=${callbackURL}?token=${token}>...</a>"})
  R-->>BA: ok
  BA-->>A: { ok: true }
  A-->>W: 200
  W->>U: 「メール送信しました」表示
...
- `setupGuard` middleware: `@setup` 付き route で、User が以下を満たさない場合 `403 SETUP_REQUIRED`:
  1. UserTimetable が 1 つ以上ある (= School/Department が間接的に確定)
  2. または、`User.defaultSemesterId != null` かつそれに紐付く UserTimetable が存在
...
13. setup 未完了 (UserTimetable が 0 件) のユーザーが `GET /api/today` を叩くと `403 { code: "SETUP_REQUIRED" }`
14. setup 未完了で `GET /api/user-timetables/:id` を叩くと `403 { code: "SETUP_REQUIRED" }`
15. setup 未完了でも `GET /api/me` `GET /api/schools` `POST /api/semesters` `POST /api/user-timetables` 等は 200 (setup 操作自身は通す)
16. `GET /api/me` のレスポンス `setupStatus.isComplete` は (a) UserTimetable >= 1 のとき true、(b) それ未満で false
...
38. `POST /api/user-timetables/:id/publish-as-template` は user の School+Department を直近 setup から導出して Template を新規作成、`copyCount=0`
...
75. `sendMagicLink` callback 内で `resend.emails.send` を呼ぶ際、`from=process.env.RESEND_FROM`、`to=email`、`subject` 固定、`html` 内に `${callbackURL}?token=${token}` を含む
