...
- **バージョン更新: 版数の正典は `project.yml` の `info.properties` 一択** (現在 `CFBundleVersion: "6"` / `CFBundleShortVersionString: "1.0"`。**build 6 = 2026-07-19 に P1〜P3+polish 全部入りで TestFlight 配布済**、ASC 上の最新)。次ビルドは `"7"` にインクリメントする。ASC の最新 build 番号は `~/.appstoreconnect` の鍵で `GET /v1/builds?filter[app]=6790604371&sort=-version` で確認できる
