...
- **バージョン更新: 版数の正典は `project.yml` の `info.properties` 一択** (現在 `CFBundleVersion: "7"` / `CFBundleShortVersionString: "1.0"`。**build 7 = 2026-07-21 に UI 統一分も入れて TestFlight 配布済**、ASC 上の最新)。次ビルドは `"8"` にインクリメントする。ASC の最新 build 番号は `~/.appstoreconnect` の鍵で `GET /v1/builds?filter[app]=6790604371&sort=-version` で確認できる
