...
- **バージョン更新: 版数の正典は `project.yml` の `info.properties` 一択** (現在 `CFBundleVersion: "14"` / `CFBundleShortVersionString: "1.0"`。**build 14 = 2026-07-30 に実機FBのカレンダーUI不具合5点 (罫線ズレ / 押せないセル / 青点削除 / タップ領域 / カード影の左右切れ) を入れて配布済**、ASC 上の最新。build 13 = カレンダー5点 (エラー表示 / レイヤー / タップ範囲 / ヘッダー)。build 12 = 個人予定を instant 化 (破壊的 migration)。build 11 = カレンダーUI詰め5点。build 10 = EventKit双方向同期 + ルーム共有マスキング、build 9 = ホーム折りたたみ刷新 + 3バグ修正。**backend 依存を含むビルドは出荷時に atender-api を Coolify デプロイすること**)。次ビルドは `"15"` にインクリメントする。ASC の最新 build 番号は `~/.appstoreconnect` の鍵で `GET /v1/builds?filter[app]=6790604371&sort=-version` で確認できる
