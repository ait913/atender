...
export function registerCourseRoutes(app: Hono) {
  app.post("/api/courses", sessionMiddleware, setupGuard, zJson(CourseCreateInput), async (c) => {
  ...
  app.patch("/api/courses/:id", sessionMiddleware, setupGuard, zParam(IdParam), zJson(CourseUpdateInput), async (c) => {
    const user = c.get("user");
  ...
  app.delete("/api/courses/:id", sessionMiddleware, setupGuard, zParam(IdParam), zQuery(DeleteCourseQuery), async (c) => {
    const user = c.get("user");
    const { id } = c.req.valid("param");
