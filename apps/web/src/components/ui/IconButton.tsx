...
const variants: Record<IconButtonVariant, string> = {
  ...
  danger: "bg-red-50 text-status-absent hover:bg-red-100",
};
...
export function IconButton({ label, icon, variant = "plain", className, type = "button", ...props }: IconButtonProps) {
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={cx("inline-flex min-h-11 min-w-11 items-center justify-center rounded-md transition disabled:opacity-50", variants[variant], className)}
      {...props}
    >
      {icon}
    </button>
