...
export function TimetableSettingsSheet({ open, onClose, userTimetable }: { open: boolean; onClose: () => void; userTimetable: UserTimetableDto | null }) {
  ...
  const bulk = useBulkReplaceDaySlots(userTimetable?.id);
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title="時間割設定">
      <div className="grid gap-4">
        {error ? <p className="rounded-md bg-red-50 px-3 py-2 text-sm font-semibold text-status-absent">{error}</p> : null}
        <Field label="時限数">
          <Input type="number" min={1} max={12} value={slots.length} onChange={(event) => resize(Number(event.target.value))} />
        </Field>
        <div className="grid gap-3">
          {slots.map((slot, index) => (
            <div key={slot.periodIndex} className="grid grid-cols-[1fr_1fr_1fr] gap-2">
              <Input value={slot.label} onChange={(event) => setSlots((current) => current.map((item, i) => (i === index ? { ...item, label: event.target.value } : item)))} />
              <Input type="time" step={60} value={minutesToTime(slot.startMinute)} onChange={(event) => setSlots((current) => current.map((item, i) => (i === index ? { ...item, startMinute: timeToMinutes(event.target.value) } : item)))} />
              <Input type="time" step={60} value={minutesToTime(slot.endMinute)} onChange={(event) => setSlots((current) => current.map((item, i) => (i === index ? { ...item, endMinute: timeToMinutes(event.target.value) } : item)))} />
            </div>
          ))}
        </div>
        <Button variant="secondary" onClick={() => resize(5)}>標準時刻に戻す</Button>
        <Button
          onClick={() => {
            setError(null);
            bulk.mutate(
