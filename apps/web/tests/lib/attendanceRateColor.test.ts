...
describe("rateColor", () => {
  it("maps null and required-rate thresholds to semantic color tokens", () => {
    ...
    expect(rateColor(70, 70)).toBe("var(--color-status-present)");
    expect(rateColor(69.9, 70)).toBe("var(--color-status-tardy)");
    expect(rateColor(59, 70)).toBe("var(--color-status-absent)");
    expect(rateColor(95, 90)).toBe("var(--color-status-present)");
  });
});
